# Next Steps: Train DeepSeek on CAMs

Goal: use Stage 1 CAMs as pseudo-labels to train a DeepSeek vision(-language) model, as described in the paper (arXiv:2503.12068).
...
- Access to DeepSeek weights/checkpoints and training code (vision or vision-language variant specified by the paper).
...
4) **Convert to training format** expected by DeepSeek codebase:
...
## Training DeepSeek (high-level checklist)
- **Model init**: load the DeepSeek variant used in the paper (e.g., vision-only vs VLM). Freeze or finetune text tower per paper guidance.
...
2) Implement a converter from CAM outputs to the DeepSeek training format (masks/boxes + manifest files).
3) Draft DeepSeek training config mirroring the paper; run a short sanity-training to ensure loss decreases.
...
- Exact DeepSeek variant, input resolution, and loss functions.
- Confidence thresholding and post-processing used on CAMs.
- Any text supervision or prompts paired with CAM regions.

Document owners can fill these with specifics after aligning with the paper or released DeepSeek training scripts.
